...
void* dmalloc_init(void* new_space, size_t sz, const char* file, long long line){
    ...
    void* underflow_canary = (void*)(((uintptr_t)new_space) + sizeof(header));
    memcpy(underflow_canary, &canary_value, sizeof(canary_value));
    // Set overflow canary
    void* overflow_canary = (void*)((uintptr_t)underflow_canary + (sizeof(canary_value)) + sz);
    memcpy(overflow_canary, &canary_value, sizeof(canary_value));
...
bool check_canary(void* new_space, size_t sz){
    // Detect overflow
    void* underflow_canary = (void*)(((uintptr_t)new_space) - (sizeof(canary_value)));
    void* overflow_canary = (void*)(((uintptr_t)new_space) + sz);
    size_t under, over;
    memcpy(&over, overflow_canary, sizeof(canary_value));
    memcpy(&under, underflow_canary, sizeof(canary_value));
    //printf("Underflow: %zu\n", under);
    //printf("Overflow: %zu\n", over);
    if ((under != canary_value) ||  (over != canary_value))
    {
        return false;
    }
